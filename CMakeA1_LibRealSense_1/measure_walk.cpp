...
void Custom::RobotControl() 
{
    ...
    rs2::pipeline p;
	p.start();
    std::cout << "Pipeline Started" << endl;
    ...
    std::cout << "dist: " << dist << "meters \r";
    cmd.mode = 1;
    /*cmd.roll = 0.5f;
    usleep(.25*micro);
    cmd.mode = 1;
    cmd.pitch = 0.3f;
    usleep(.25*micro);//delay
    cmd.mode = 1;
    cmd.yaw = 0.3f;
    usleep(.25*micro);//delay
    cmd.mode = 1;
    ...
    else if (dist < 1.50 && dist > 1.0)
    {
        cmd.mode = 1;
        cmd.bodyHeight = -0.5f;
    }
    else if (dist < 1.0 && dist > .50)
    {
        
    cmd.mode = 2;
    cmd.forwardSpeed = -0.3f; // -1  ~ +1       
    }
    else if (dist < .50)
    {
        
    cmd.mode = 2;
    cmd.forwardSpeed = 0.0f; // -1  ~ +1       
    }
